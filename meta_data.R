...
#### input needs to be the security definition file
#### dependents: data.table stringr

meta_data <- function(sunday_raw_data_path, date, security=NULL){
  ...
  data <-  fread(sunday_raw_data_path, header = F, sep="\\", fill=TRUE)[[1L]]
  meta_data <- main(data, date = date, security = NULL)
